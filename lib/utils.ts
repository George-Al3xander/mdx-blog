...
import { SortFilter, TMainPageWrapperProps } from "@/types/types";
...
export const addSearchParam = (
    ...
    if (typeof oldSearchParams === "string") {
        oldSearchParams = new URLSearchParams(oldSearchParams);
    }
    if (
        typeof oldSearchParams === "object" &&
        !(oldSearchParams instanceof URLSearchParams)
    ) {
        oldSearchParams = objectToUrlSearchParams(oldSearchParams);
    }
...
export const destructMainPagesProps = ({
    searchParams = { page: "1" },
}: TMainPageWrapperProps) => {
    const { searchQuery, sortBy, page = "1" } = searchParams;

    return { searchQuery, sortBy, page };
};
